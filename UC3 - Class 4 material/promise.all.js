...
Promise.all([
  ...
  var postResponse = responses[0];
  var commentReponse = responses[1];
  postCallback(postResponse);
  commentCallback(commentReponse);
  albumCallback(responses[2]);
  photoCallback(responses[3]);
  todoCallback(responses[4]);
  userCallback(responses[5]);
});
